...
How does it work?  
We need to learn some calculus; but you are not here for that. Let's tackle if differently: Y-Combinator The hard way.

Let's move back a bit. Imagine a function.  
We want more of it.  
...
Forever execution! We are getting there.  
...
We need a function to call, and cannot pass reference it by name. Alternative?  
...
1. know we should pass `factorial` to `loop` to execute it on `loop(factorial)(5)`. It belongs to `factorial` implementation.
2. mix `loop` with `factorial`'s internal logic. `loop(fact)(n-1)` is mixing different matters; it's noisy.
